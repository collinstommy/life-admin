...
import { app } from './app';
import { NotionApiClient } from '../server/src/api/notion';
import type { MiddlewareHandler } from 'hono';
import { AppContext, HonoApp } from './types';
import { sign, verify } from 'hono/jwt';
import { getCookie, setCookie } from 'hono/cookie';
import { storeAudioRecording, getAudioRecording } from '../server/src/lib/storage';
import { transcribeAudio, extractHealthData } from '../server/src/lib/ai';
// We'll use these in Phase 2 with database integration
import { getAllHealthLogs, getHealthLogById, initDb } from '../server/src/lib/db';
// Import the saveHealthLog function
import { saveHealthLog } from '../server/src/lib/db';
...
app.post('/api/process-transcript', async (c) => {
  try {
    ...
    try {
      ...
      // Save to database
      const logId = await saveHealthLog(c as AppContext, '', transcript, healthData);
...
app.post('/api/health-log', async (c) => {
  try {
    ...
    try {
      ...
      console.error('Error in audio processing or transcription:', transcriptError);
      return c.json(
        {
    ...
    try {
      ...
      const logId = await saveHealthLog(c as AppContext, audioUrl, transcript, healthData);
      console.log('Health log saved to database with ID:', logId);
  ...
  try {
    ...
    try {
      ...
      if (logs.length === 0) {
        ...
        return c.json({
          logs: [],
          message: 'No health logs found. Create your first health log by recording or entering a transcript.',
        });
      }
      ...
      if (dbError.message && dbError.message.includes('no such table')) {
        console.log("Health logs table doesn't exist yet, returning empty result");
        return c.json({
          logs: [],
          message:
...
app.get('/logs', async (c) => {
  const notionClient = new NotionApiClient(c.env.NOTION_TOKEN, c.env.NOTION_DATABASE_ID, c.env.DAILY_LOG_CACHE);
